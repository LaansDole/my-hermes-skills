...
- Ensure the list exists: `remindctl list "Hermes Follow-up" --create` (idempotent).
- For each NEW task: `remindctl add --title "<title>" --list "Hermes Follow-up" --due "<YYYY-MM-DD HH:mm>"` — always a concrete date string (see pitfall 6).
- Skip any task whose title already has an open reminder (check `remindctl list "Hermes Follow-up" --json`).
...
2. For each task with `due <= now` (due today or overdue): ensure an open reminder exists — if its title is missing from `remindctl list "Hermes Follow-up" --json`, create it with `--due "<today's date YYYY-MM-DD> 09:00"` (always a concrete date string — see pitfall 6).
