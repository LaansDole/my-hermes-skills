...
- **Review agents wired to recall (PR-review use-case)**: reviewer/reviewer-deep/pr (`~/.omp/agent/agents/*.md`) gained MCP tools + a consult step. Exact tool keys omp registers (server name sanitized to underscores): `mcp__omp_episodic_memory_recall_for_task`, `mcp__omp_episodic_memory_search`, `mcp__omp_episodic_memory_list_gotchas` (+ `read`, `get_project_context`). Format = `mcp__<server_underscored>_<tool>` (single `_` separator). Purpose: PR reviews recall prior decisions/gotchas from past session transcripts (esp. folktale). Sync edits back to the `omp-preset` repo agents/ dir (source of truth, bootstrap.sh installs them).
