...
- The "context zone" statusline idea (r/PiCodingAgent statusline-pi: tells you when to plan/code/new-session) is best implemented in omp's native `statusLine.*` — NOT a custom extension. **We tried a custom `ctx.ui.setStatus()` extension (`omp-statusline`) and DROPPED it (2026-08-31):** setStatus can re-expose only what ctx holds (model, context %, cost) plus a zone label (a restyle of context%) — all already shown by the native bar — and cannot reach ccstatusline's richer widgets (cache hit rate, granular usage) because an omp extension's `ExtensionContext` exposes no settings/store and no bonus metrics. Net effect was pure duplication with zero added value, so the extension + its config-file plumbing + its `/statusline` command were removed; the native config is the statusline of record.
- **`ctx.ui.theme.fg(style, text)` takes ONLY valid ThemeColor names** — passing an unknown one throws `Unknown theme color: XYZ` on `session_start` (caught via RPC `extension_error`). Valid set (verified): `text, error, dim, accent, muted, success, warning, border, info, primary, subtle, secondary, foreground, background, brand`. Do NOT use `model/path/context/cost`.
- **`ctx.ui.setStatus(key, text)` renders** (a `"statusKey"` frame appears in the RPC stream and renders in the TUI; works even in RPC mode) — this is the ONLY extension mechanism omp renders a statusline through (`setFooter` is ignored).
- **`ExtensionContext` has NO settings/store** — no `ctx.settings`, no `getStorage`. To read a config file an extension must ship compiled `.js` (bun build) so `node:fs` works (the no-node:fs rule binds only omp-compiled `.ts`). `ExtensionUIContext.select`/`input`/`confirm` are the sanctioned user-dialog APIs.
- **Built via `omp -p`; no standalone tsc gate.** omp-compiled extensions use `@earendil-works/pi-coding-agent` as a **peerDependency resolved by omp's runtime** (follow omp-tldr: no `npm install`, no `npx tsc`). Real gates: `omp plugin install`/`link`, `omp plugin doctor`, RPC smoke. RPC smoke pitfall on macOS: hold stdin open — `( sleep 8 | omp --mode rpc --no-session > log 2>&1 & )` — else the log is empty.
- **Dispatch mechanics for a new local extension repo:** write the plan to `docs/superpowers/plans/`, then `omp -p --cwd=/Users/laansdole/Projects <pkg> @<dispatch>.md` in background with notify. Create the repo as a sibling, git init + commit locally, do NOT push unless asked. omp -p WILL stop on a hard gate — corrected re-dispatch, don't let it improvise. Read the real `.d.ts` types in `~/.omp/plugins/node_modules/@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts` before writing a plan (authoritative, beats bundle-string guessing).
