...
- The "context zone" statusline idea (r/PiCodingAgent statusline-pi: tells you when to plan/code/new-session) is best implemented in omp's native `statusLine.*`, or as an extension using **`ctx.ui.setStatus()`** (the mechanism ponytail/i-have-adhd actually use that omp renders) — not `setFooter`. BUILT 2026-08-30: `~/Projects/omp-statusline` (local, wired into omp-preset plugins.json as optional local ref). Single-file extension, subscribes `session_start`/`turn_end`/`session_compact`/`model_select`, calls `ctx.ui.setStatus("omp-statusline", ...)`. Renders `Claude Fable 5 ┃ ctx 3% [code] ┃ /tmp` verified live.
- **`ctx.ui.theme.fg(style, text)` takes ONLY valid ThemeColor names** — passing an unknown one throws `Unknown theme color: XYZ` on `session_start` (caught via RPC `extension_error`). Valid set (verified in bundle): `text, error, dim, accent, muted, success, warning, border, info, primary, subtle, secondary, foreground, background, brand`. Do NOT use `model/path/context/cost` as theme colors — map model→`accent`, ctx→`text`, cwd→`dim`, cost→`text`.
- Status line footer pieces CAN also go through `ctx.ui.setStatus(key, text)` — a `"statusKey":"omp-statusline"` frame appears in the RPC stream and renders colored in the TUI (works even in `--mode rpc`).
- **Built via `omp -p`; no standalone tsc gate.** omp-compiled extensions use `@earendil-works/pi-coding-agent` as a **peerDependency resolved by omp's runtime** (follow omp-tldr: no `npm install`, no `npx tsc`). The real gates: `omp plugin install`/`link`, `omp plugin doctor`, RPC smoke (grep for `"statusKey":"<key>"` AND confirm ZERO `extension .* error` lines). RPC smoke pitfall: `setStatus` frames carry a `statusKey` (not like commands), and RPC mode has `ctx.hasUI` true enough to render.
- **Dispatch mechanics for a new local extension repo:** write the plan to `docs/superpowers/plans/`, then `omp -p --cwd=$HOME/Projects @<dispatch-file>.md` in background with notify. Create the new repo as a sibling (`~/Projects/<pkg>`), git init + commit locally, do NOT push unless asked. omp -p WILL stop on a hard gate — prefer a corrected re-dispatch over letting it improvise.
