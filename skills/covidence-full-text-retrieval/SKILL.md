...
- **Tool-name mapping**: this skill's steps are written against a generic `tab.observe()/tab.evaluate()/tab.uploadFile()/tab.fill()` vocabulary. On Hermes, map these to the actual toolset: `tab.observe()` -> `browser_snapshot`; `tab.evaluate(fn)` -> `browser_console` (pass the JS as an expression string, e.g. `location.href` or an IIFE); `tab.screenshot()` -> `browser_vision`; `tab.fill(el, text)` -> `browser_type` when a stable `@eN` ref exists, otherwise `browser_console` with a native-setter + `dispatchEvent(new Event('input', {bubbles:true}))` pattern (React-controlled inputs ignore a raw `.value =` assignment). There is no `tab.uploadFile` equivalent in this Hermes CDP setup -- see the NotebookLM Preflight and Action Policy sections below for the actual upload mechanism.
- **NotebookLM preflight (do this BEFORE starting the Screen Loop, not lazily on first Layer 4 miss)**: if `notebooklm_topic` is set, navigate a spare tab to `notebooklm.google.com` right away, before touching Covidence. If it redirects to a Google sign-in page, STOP and tell the user: "NotebookLM isn't authenticated in this CDP-attached browser profile -- please log in at notebooklm.google.com in this same browser/profile, then tell me to continue." Wait for their go-ahead, then re-check. Only proceed to the main Screen Loop once NotebookLM is confirmed authenticated (or the user explicitly says to proceed without Layer 4). This avoids burning through several real references before discovering deep into a run that Layer 4 was never going to work.
...
| `browser_navigate` false-timeout | `browser_navigate` returns `"Operation timed out"` even though the navigation actually completed | This is a known false-negative on this Hermes CDP setup, not a real failure. Immediately check `location.href` via `browser_console` before treating it as an error -- if the URL matches the intended destination, proceed normally; only escalate to a real error if the URL is still stale after this check. |
