...
- [`plugins/chrome-profiles/`](plugins/chrome-profiles/README.md) -- Hermes Agent plugin: switch the agent's browser tools between multiple Chrome, Brave, or Edge instances via CDP. Each profile needs its own `data_dir` (dedicated user-data-dir) -- it launches a browser process, it does not select a named profile out of an already-running shared browser. Install with `hermes plugins install LaansDole/my-hermes-skills/plugins/chrome-profiles`.